...
export const EMPTY_PIXEL = "data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=";
...
class Image extends Component {
  ...
  */
  ...
  render() {
    ...
    const loadSrc = (!ondemand && !lazy) || this.state.mounted ? src : EMPTY_PIXEL;
    return (
      <img
        {...other}
        itemProp="image"
        src={loadSrc}
        onClick={_onClick}
        className={classNames(extras, className, hidden ? "hide-content" : "")}
      >
        {children}
      </img>
    );
...
const validateDimension = props => {
  return (
    ...
    new Error(
      `If you use lazy (set to ${props.lazy} ) you should
